...
pos_curve = gcurve(volor = color.blue, label = 'x-position', graph = pos_graph)

#Decorrer do tempo no gráfico
#vel_graph = graph(xtitle = "Tempo", ytitle = "Velocidade")
#vel_curve = gcurve(color = color.red, label = "x-velocity", graph = vel_graph)

y_curve = gcurve(color = color.black, label = "y-position", graph = pos_graph)
#v_y = gcurve(color = color.purple, label = "y-velocity", graph = vel_graph)
